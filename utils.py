...
def fetch_and_store_data():
    ...
    for page in range(1, total_pages + 1):
        ...
        for tool_data in page_data:
            existing_user = (
                session.query(User)
                .filter(User.username == tool_data["author"][0]["name"])
                .first()
            )
            ...
            tool = Tool(
                ...
                url=tool_data["url"],
                keywords=tool_data.get(
                    "keywords", ""
                ),  # Use .get() to handle missing keys
                author=tool_data["author"][0]["name"],
                repository=tool_data.get(
                    "repository", ""
                ),  # Use .get() to handle missing keys
                license=tool_data.get(
                    "license", ""
                ),  # Use .get() to handle missing keys
                technology_used=", ".join(
                    tool_data.get("technology_used", [])
                ),  # Use .get() to handle missing keys
                bugtracker_url=tool_data.get(
                    "bugtracker_url", ""
                ),  # Use .get() to handle missing keys
                page_num=page,
    ...
    for tool in tools:
        ...
        if tool.health_status is False:
            tool_pref = (
                ...
                .filter(Tool_preferences.tool_id == tool.id)
                .first()
            )
            last_up = (
            ...
            if last_up is not None:
                ...
                if (
                    tool_pref.interval * 60
                    >= (
                        (datetime.datetime.now() - last_up.timestamp).total_seconds()
                        + 1800
                    )
                    and tool_pref.interval != 0
                ):
                    send_email(tool_pref.user_name, tool_pref.tool.name)


def set_interval(interval, tool):
    session = Session()
    tool = (
        session.query(Tool_preferences).filter(Tool_preferences.tool_id == tool).first()
    )
    tool.interval = interval
    session.commit()


def send_email(username, tool_name):
    print(f"Sending email to {username}")
    yag = yagmail.SMTP(
        user="", password="", port=465
    )  # (email, password)
    yag.send(
        to=f"{tool_name}.notifications@toolforge.org",
        subject=f"Your Wikimedia Tool: {tool_name} is down!",
        contents=f"Your tool: {tool_name} went down at {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\nYou are recieving this message because you have opted in to recieve email notifications when your tool goes down on https://tool-watch.toolforge.org/",
    )
